...
export default function Navbar() {
  ...
  return <AppBar position="sticky" elevation={0} sx={{ bgcolor: "rgba(255,255,255,.92)", color: "text.primary", backdropFilter: "blur(16px)", borderBottom: "1px solid #e9eef5" }}>
    <Container><Toolbar disableGutters sx={{ minHeight: { xs: 64, md: 76 } }}>
      ...
      <Stack direction="row" spacing={.5} sx={{ display: { xs: "none", md: "flex" }, alignItems: "center" }}>
        {navItems.map(([label, path]) => <Button key={path} component={Link} to={path} color="inherit" sx={{ color: location.pathname === path ? "primary.main" : "text.secondary", px: 1.35 }}>{label}</Button>)}
        <Button component={Link} to="/contact" variant="contained" endIcon={<ArrowForwardIcon />} sx={{ ml: 1.5, px: 2 }}>Let's talk</Button>
      </Stack>
      <IconButton onClick={e => setAnchorEl(e.currentTarget)} sx={{ display: { md: "none" }, color: "primary.main" }} aria-label="Open navigation"><MenuIcon /></IconButton>
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)} PaperProps={{ sx: { mt: 1, minWidth: 220, borderRadius: 3 } }}>
        {[...navItems, ["Contact", "/contact"]].map(([label, path]) => <MenuItem key={path} component={Link} to={path} selected={location.pathname === path} onClick={() => setAnchorEl(null)} sx={{ py: 1.25 }}>{label}</MenuItem>)}
      </Menu>
    </Toolbar></Container>
  </AppBar>;
}
